...
```xml
<servlet>
    <setvlet-name>서블릿 이름</servlet-name>
    <servlet-class>서블릿 클래스</servelt-class>
</servlet>
<servlet-mapping>
...
- 서블릿이 웹상에서 HTTP 프로토콜을 이용해 서비스를 위해 반드시 상속해야 하는 클래스
...
- getParmeterValues(String) : client가 보내준 값이 여러개 일 경우 그값을 배열로 불러오는 메소드
...
- setContentType(Stirng) : 응답으로 작성하는 페이지의 MIME type(파일변환)을 정하는 메소드
...
1. 첫번째 요청일 경우, 서블릿 객체를 생성하고 init() 메소드를 호출한다.
   (단, 한번만 수행)
2. service() 메서드가 클라이언트 요청 방식에 따라 doGet(), doPost() 메소드를 호출한다.
...
- 표현식(Expression) 태그를 대신하여 클라이언트에 출력하고자 하는 값들을 좀 더 간경하게 사용하는 방법
...
- applicationScope : Application 영역 객체에 접근(Context)
...
- 액션 태그의 경우 웹 브라우저에서 실행되는 것이 아니라, 웹 컨테이너에서 실행
- 표준 액션 태그(forward) : JSP 페이지에서 바로 사용, 태그 앞에 jsp 접두어 붙음
- 커스텀 액션 태그(c) : 별도의 라이브러리 설치 필요, 라이브러리 선언에 맞는 접두어가 붙음

### JSTL (JSP Standard Tag Library)

- JSP에서 사용하는 커스텀 태그
- JSP 페이지에서 자주 사용하는 코드들을 사용하기 쉽게 태그로 만들어 표준으로 제공
...
- 변수의 자료형은 별도로 선언하지 않지만 초기값은 반드시 기술해야한다.
...
- 조건식은 결과가 참일 때 `<c:if>~</c:if>`내용을 처리한다.
...
- 하위 태그인 `<c:when>`,`<c:otherwise>` 태그와 함께 사용되는데, 각각 switch 구문의 case, defalut 절과 비숫한 역할을 한다.

```jsp
<c:choose>
    <c:when test="${num eq 0}"></c:when>
    <c:when test="${num eq 0=1}"></c:when>
...
- 전체 페이지를 갱신하지 않고 일부분만 업데이터가 가능하며 사용자에게 즉각적인 반응과 풍부한 UI 경험 제공
- JavaScript 또는 jQuery 방식으로 구현

### 단점

- Ajax는 Javascript이므로 브라우저에 따른 크로스 브라우저 처리 필요
- 연속적인 데이터 요청시 서버 부하 증가하여 페이지가 느려짐
...
1. 객체 생성 : script문에 요청을 위한 XTMLHttpRequest 객체 생성
...
```js
$.ajax({
    url: "요청이 전송되는 url이 포함된 문자열(필수 구현 속성,servlet)",
    [settings..]
});
```
...
- complete : ajax 통신 성공 여부와 관계없이 통신이 완료 후 실행되는 함수 지정
- async : 비동기(true) / 동기(false) 지정

### 장점

- 코드길이 감소. JavaScript 방식보다 구현 방법이 간단
- 직관적이며 다양한 방법의 코딩 가능
...
- 객처 {} 또는 배열 [] 데이터를 효율적으로 표시 가능

### 특장

- Ajax 통신에서 Object 타입의 데이터 전송 시 XML 대비 용량이 작고 속도가 빠름(경량 데이터 교환 방식)
- 간단한 포맷을 가지고 있어 이해하기 쉬움
- 순수 TEXT 기반으로 TEXT형식이 구조화 되어 있으며 대부분의 프로그래밍 언어에서 JSON 포맷 데이터를 핸들링할 수 있는 라이브러리를 제공해 시스템 간 객체 교화에 용이
