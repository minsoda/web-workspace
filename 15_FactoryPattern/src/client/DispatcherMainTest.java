...
public class DispatcherMainTest {
		 ...
		 * 
		 * 1) 구상객체를 실질적으로 생성하는 곳은 싱글톤 패턴으로 작성된 Factory 에서 진행.
		 * 		이때, 인자값으로 넘겨져오는 command값에 따라서 서로 각각 다른 구상객체가 만들어진다.
		 * 		하지만 정작 사용자에게는 구상객체명이 hiding(보이지않게) 되어져서 넘겨진다.
		 * 		--> 추상화된 클래스(Action)으로 날라간다.
		 * 		--> 결국 클라이언트는 Factory로 하여금 구상객체를 만들도록 요청하는 것이고
		 * 		Factory(공장이라고 생각하세용!)는 요청에 해당하는 제품을 만들고 인터페이스로 전달.
		 * 
		 * 2) 인터페이스의 메소드를 호출하면
		 * 		실질적으로 생성된 구상객체가 드디어 핵심적인 일(비지니스로직 호출)을 진행!
