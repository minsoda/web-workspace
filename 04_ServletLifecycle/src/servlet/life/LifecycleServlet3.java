...
/*
 	서버가 stop 되는 순간 destory() callback 함수가 자동으로 호출되어 count 값이 영구적으로 보관x
 	
 	보관을 하려면 DB or 파일로 서버가 내려가도 서비스를 수행한 후 필드에 저장시킨 값을 저장
...
public class LifecycleServlet3 extends HttpServlet {
	private static final long serialVersionUID = 1L;
       public int count = 0;
       private String path = "d:\\test\\count.txt"; // file 때문에 경로 하나 만들어주기!
	...
	public void init(ServletConfig config) throws ServletException {
		...
		// 서버가 다시 시작될때 init에서 파일에 저장된 내용을 읽어서 count에 다시 할당
		try {
			BufferedReader br = new BufferedReader( new FileReader(path));
			String str = br.readLine();
			count = Integer.parseInt(str); // 숫자값으로 변경
			br.close();
			
			System.out.println("count.txt 파일의 내용을 읽어들임... count :: " + count);
		} catch (IOException e) { // 두번쨰 try catch
...
//		필드에 저장된 값을 파일로 저장하고 서버가 내려질 것
		...
		// 출력용 파일을 출력스트림에서 자동적으로 생성되려면 그 전에 반드시 디렉토리는 만들어져 있어야 한다.
